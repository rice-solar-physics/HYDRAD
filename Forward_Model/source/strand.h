...
class CStrand {
    ...
    // Pointer to an array of structures containing the physical properties
    // of the strand as a function of position in field-aligned coordinates
    PPHYDATA pPHYData;

    // The number of non-equilibrium (NEQ) elements and their atomic numbers
    int iNumNEQElements, *piNEQ_Z;

    // The non-equilibrium population fractions
    // pppNEQ[X][Y][Z], where X = cell, Y = element and Z = ion
    double ***pppfNEQ;
    ...
    // non-equilibrium ionisation state exists
    ...
    // specified ion / element for a specified cell
